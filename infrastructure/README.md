...
2. **Tools**
   - Terraform >= 1.0.0
   - AWS CLI >= 2.0.0
   - Harbor registry (self-hosted or existing) with a project and robot account

3. **Application image**
   - Build and push the Java app image before applying ASG:
     ```bash
     cp .env.example .env
     ./scripts/harbor-build-push.sh
     ```
...
2. **Configure Variables**
   ...
   harbor_project    = "java-login-app"
   harbor_image      = "dptweb"
   harbor_image_tag  = "1.0"
   harbor_username   = "robot$java-login-app+deploy"
   harbor_password   = "your-harbor-robot-token"
   harbor_insecure   = false
   ```

   Or copy `terraform.tfvars.example` to `terraform.tfvars`. EC2 instances pull `harbor.example.com/java-login-app/dptweb:1.0` and run it with Docker.
